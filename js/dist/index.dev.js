...
(function ($) {
  ...
  function Slider(ele, opt) {
    ...
    this.opts = $.extend({}, d, opt); //传递过来的的元素
    ...
    this.nowIndex = 0;
    this.autoPlay = this.opts.autoPlay; //计时器函数初始为空
  ...
  Slider.prototype.initMove = function () {
    ...
    for (var i = 0; i < hLen; i++) {
      lNums = self.curDisplay - i - 1; //console.log(lNums)

      self.$img.eq(lNums).css({
        transform: "translateX(".concat(-150 * (i + 1), "px) translateZ(").concat(200 - i * 100, "px) rotateY(", 30, "deg)")
      });
      rNums = self.curDisplay + i + 1;
      self.$img.eq(rNums).css({
        transform: "translateX(".concat(150 * (i + 1), "px) translateZ(").concat(200 - i * 100, "px) rotateY(").concat(-30, "deg)")
      });
    }
